// DENTRO DE: src/pages/CasosDeAgravamentoPage.jsx
...
// Componente de Badge para exibir o tipo de alteração
...
export default function CasosDeAgravamentoPage() {
  const { companyId } = useParams();
  const { casosDeAgravamento, totalCasosDeAgravamento, areAgravamentosLoading, fetchCasosDeAgravamento } = useData();
  
  const [currentPage, setCurrentPage] = useState(1);

  useEffect(() => {
    if (companyId) {
      fetchCasosDeAgravamento(companyId, currentPage - 1, PAGE_SIZE);
    }
  }, [companyId, currentPage, fetchCasosDeAgravamento]);

  const totalPages = Math.ceil(totalCasosDeAgravamento / PAGE_SIZE);

  if (areAgravamentosLoading) {
    return (
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <h1 className="text-3xl font-bold text-slate-800">Casos de Agravamento Confirmado</h1>
        <p className="text-gray-500 mt-1">
          Exibindo {casosDeAgravamento.length} de {totalCasosDeAgravamento} caso(s).
        </p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200">
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody>
              {casosDeAgravamento.length > 0 ? (
                casosDeAgravamento.map(caso => (
                ...
                <tr>
                  <td colSpan="3" className="text-center p-8 text-gray-500">
                    Nenhum caso de agravamento confirmado encontrado.
                  </td>
                </tr>
              )}
            </tbody>
